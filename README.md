...
- **`ADMIN_KEY` を必ず設定してください。**
  ...
  何も対策しないと外部からのアクセスも接続元 `127.0.0.1` に見えてしまいます。
  本サーバーは `CF-Ray` や `X-Forwarded-For` などのヘッダが付いていれば
  プロキシ経由と判断して localhost 扱いにしませんが、
  合言葉を設定しておくのが確実です。設定しないと運営自身も別端末から操作できません。
